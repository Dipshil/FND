...
class LIWC:
    ...
    def __init__(self, ngram_limit=2, min_df=2, is_sublinear_tf=True):
        self.tfidf_corp = TF_IDF_Corpus(ngram_limit=2, min_df=2, is_sublinear_tf=True)
    ...
    def parse_liwc_results(self, file_path, gram=False, neg=False):
        ...
        with open(file_path, 'r') as f:
            ...
            if gram: cat_list = f.readline().strip().split("\t")[1:22]
            elif neg: cat_list = f.readline().strip().split("\t")[24:28]
            else: cat_list = f.readline().strip().split("\t")

            for line in f:
                line = line.strip().split("\t")
                token = line[0]

                if gram: occurences = np.where(np.asarray(line[1:22]) == 'X')[0]
                elif neg: occurences = np.where(np.asarray(line[24:28]) == 'X')[0]
                else: occurences = np.where(np.asarray(line[1:]) == 'X')[0]
                cats = np.asarray(cat_list)[occurences]

                for cat in cats: categories[token][cat] = cat_list.index(cat)
    ...
    def get_gram_vectors(self):
        train_gram = self.gram_vectorize(paths['TRAIN'], paths['TRAIN_RES'])
        valid_gram = self.gram_vectorize(paths['VALID'], paths['VALID_RES'])
        test_gram = self.gram_vectorize(paths['TEST'], paths['TEST_RES'])

        return train_gram, valid_gram, test_gram


    def get_neg_vectors(self):
        train_neg = self.neg_vectorize(paths['TRAIN'], paths['TRAIN_RES'])
        valid_neg = self.neg_vectorize(paths['VALID'], paths['VALID_RES'])
